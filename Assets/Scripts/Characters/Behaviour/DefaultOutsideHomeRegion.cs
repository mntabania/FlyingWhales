        ...
        if (!character.isAtHomeRegion) {
            ...
            if (currentTimeOfDay == TIME_IN_WORDS.MORNING || currentTimeOfDay == TIME_IN_WORDS.LUNCH_TIME || currentTimeOfDay == TIME_IN_WORDS.AFTERNOON || currentTimeOfDay == TIME_IN_WORDS.EARLY_NIGHT) {
                ...
                if (!character.currentStructure.isInterior) {
                    ...
                    Area currentHex = character.gridTileLocation.area;
                    for (int i = 0; i < currentHex.neighbourComponent.neighbours.Count; i++) {
                        Area hex = currentHex.neighbourComponent.neighbours[i];
                        LocationGridTile centerTile = hex.gridTileComponent.centerGridTile;
                        //TODO: Enable digging
                    ...
                    if(structures != null && structures.Count > 0) {
                        ...
                        if (character.currentSettlement != null) {
                            ...
                            List<Campfire> campfires = currentHex.tileObjectComponent.GetTileObjectsInHexTile<Campfire>();
