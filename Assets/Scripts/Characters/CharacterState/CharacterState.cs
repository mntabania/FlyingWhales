...
public class CharacterState {
    ...
    protected virtual void CreateThoughtBubbleLog() {
        if (LocalizationManager.Instance.HasLocalizedValue("CharacterState", stateName, "thought_bubble")) {
            thoughtBubbleLog = GameManager.CreateNewLog(GameManager.Instance.Today(), "CharacterState", stateName, "thought_bubble", providedTags: characterState == CHARACTER_STATE.COMBAT ? LOG_TAG.Combat : LOG_TAG.Misc);
    ...
    private void CreateStartStateLog() {
        if (LocalizationManager.Instance.HasLocalizedValue("CharacterState", stateName, "start")) {
            Log log = GameManager.CreateNewLog(GameManager.Instance.Today(), "CharacterState", stateName, "start", providedTags: characterState == CHARACTER_STATE.COMBAT ? LOG_TAG.Combat : LOG_TAG.Misc);
    ...
    private void CreateTravellingThoughtBubbleLog(NPCSettlement targetLocation) {
        if (LocalizationManager.Instance.HasLocalizedValue("CharacterState", stateName, "thought_bubble_m")) {
            thoughtBubbleLog = GameManager.CreateNewLog(GameManager.Instance.Today(), "CharacterState", stateName, "thought_bubble_m", providedTags: characterState == CHARACTER_STATE.COMBAT ? LOG_TAG.Combat : LOG_TAG.Misc);
            thoughtBubbleLog.AddToFillers(stateComponent.owner, stateComponent.owner.name, LOG_IDENTIFIER.ACTIVE_CHARACTER);
