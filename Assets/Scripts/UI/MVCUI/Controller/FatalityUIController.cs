...
public class FatalityUIController : MVCUIController, FatalityUIView.IListener
{
	...
	private bool CanAffordSymptom(PLAGUE_FATALITY p_fatalityType) {
		if (PlayerManager.Instance != null && PlayerManager.Instance.player != null) {
			return PlayerManager.Instance.player.plagueComponent.plaguePoints >= p_fatalityType.GetFatalityCost() || (WorldSettings.Instance != null && WorldSettings.Instance.worldSettingsData.costAmount == SKILL_COST_AMOUNT.None);
		} else {
			return true;
		}
	}
