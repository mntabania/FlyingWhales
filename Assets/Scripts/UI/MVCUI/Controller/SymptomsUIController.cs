...
public class SymptomsUIController : MVCUIController, SymptomsUIView.IListener
{
	...
	private bool CanAffordSymptom(PLAGUE_SYMPTOM p_symptomType) {
		if (PlayerManager.Instance != null && PlayerManager.Instance.player != null) {
			return PlayerManager.Instance.player.plagueComponent.plaguePoints >= p_symptomType.GetSymptomCost() || (WorldSettings.Instance != null && WorldSettings.Instance.worldSettingsData.omnipotentMode);
		} else {
			return true;
		}
	}
