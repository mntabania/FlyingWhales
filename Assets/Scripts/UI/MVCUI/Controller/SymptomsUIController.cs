...
public class SymptomsUIController : MVCUIController, SymptomsUIView.IListener
{
	...
	private void UpdateSymptomData(PLAGUE_SYMPTOM p_symptomType) {
		...
		m_symptomsUIView.UpdateSymptomCost(p_symptomType, p_symptomType.GetSymptomCost().ToString());
	...
	private bool CanAffordSymptom(PLAGUE_SYMPTOM p_symptomType) {
		if (PlayerManager.Instance != null && PlayerManager.Instance.player != null) {
			return PlayerManager.Instance.player.plagueComponent.plaguePoints >= p_symptomType.GetSymptomCost();
		} else {
			return true;
		}
	}
