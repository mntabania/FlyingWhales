...
public class RainFeature : TileFeature {
    ...
    private void OnTileObjectPlaced(TileObject obj, LocationGridTile gridTile, HexTile featureOwner) {
        if(gridTile.buildSpotOwner.hexTileOwner == featureOwner) {
            if (!obj.traitContainer.HasTrait("Wet")) {
    ...
    #region Effects
    //private void ScheduleSpecificWet(ITraitable traitable) {
    //    GameDate dueDate = GameManager.Instance.Today().AddTicks(GameManager.Instance.GetTicksBasedOnMinutes(15));
    //    SchedulingManager.Instance.AddEntry(dueDate, () => CheckForSpecificWet(traitable), this);
    //}
    //private void CheckForSpecificWet(ITraitable traitable) {
